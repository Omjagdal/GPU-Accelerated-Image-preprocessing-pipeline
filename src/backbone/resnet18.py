"""
ResNet-18 Feature Extractor Backbone for Prototypical Networks.

This module provides a ResNet-18 based feature extractor that maps input images
of shape (B, 3, 224, 224) to compact embedding vectors of shape (B, 512).
It leverages transfer learning from ImageNet pretrained weights and supports
partial layer freezing for fine-tuning on domain-specific few-shot tasks
such as industrial defect detection.

Architecture Overview:
    ResNet-18 → Remove FC → AdaptiveAvgPool2d → Flatten → (B, 512) embedding

Typical Usage:
    >>> backbone = ResNet18Backbone(pretrained=True, freeze_layers=True)
...
class ResNet18Backbone(nn.Module):
    """ResNet-18 feature extractor for prototypical networks.

    Extracts 512-dimensional embedding vectors from input images using a
    pretrained ResNet-18 architecture with the final fully-connected
    classification layer removed. The adaptive average pooling layer is
    retained to produce a fixed-size spatial output regardless of input
    resolution (though 224×224 is the expected and tested input size).

    Attributes:
        features (nn.Sequential): All ResNet-18 layers up to and including
    ...
    Args:
        ...
        embedding_dim (int): Output embedding dimensionality. For ResNet-18,
        ...
        freeze_layers (bool): If True, freeze all parameters except those
            in layer3 and layer4 (the last two residual blocks). This is
            useful for fine-tuning: early layers capture generic features
            (edges, textures) that transfer well, while later layers are
            adapted to the target domain. Defaults to False.
    ...
    def __init__(
        ...
        # Load the full ResNet-18 model with or without pretrained weights.
        ...
        # ResNet-18's penultimate output is always 512-dimensional.
        ...
        if freeze_layers:
            ...
            # are highly transferable. The deeper layers (layer3, layer4)
            # learn more task-specific representations and benefit from
            # fine-tuning on the target domain (e.g., defect patterns).
